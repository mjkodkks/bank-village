...
bun run start:dev        # Dev server (auto-generates Prisma client)
bun run build            # Build
...
Scripts: `dev`, `start:dev`, `build`, `start`, `start:prod`, `lint`, `format`, `test`, `test:watch`, `test:cov`, `test:e2e`

### Frontend

```bash
cd web-client
pnpm dev       # Dev server, runs on port 3000
pnpm build     # Build
